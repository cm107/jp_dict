...
class DictionaryEntryMatch(BasicLoadableObject['DictionaryEntryMatch']):
    def __init__(self, entry: DictionaryEntry=None, search_words: List[str]=None, history_group_id_list: List[int]=None):
        self.entry = entry
...
class DictionaryEntryMatchList(
    ...
    def add_pruned_matches(self, handler: SearchWordMatchesHandler, mode: int=0, show_pbar: bool=True, leave_pbar: bool=True):
        """
        Possible modes
...
class SearchWordMatchesHandler(
    ...
    # def get_pruned_matches(self, mode: int=0, show_pbar: bool=True, leave_pbar: bool=True) -> DictionaryEntryMatchList:
    #     """
    #     Possible modes
    #         0: Ignore non-unique
    #         1: Process only first entry of non-unique. Ignore if entry already exists in matches.
    #         2: Process every entry of non-unique. Ignore if any entry already exists in matches. Otherwise add first entry.
    #         3: Process every entry of non-unique. Add first entry that doesn't already exist in matches, if any.
        
    #     Pros and Cons of each mode:
    #         0:
    #             Pros: Fast
    #             Cons: Overlooks all results that are non-unique.
    #         1:
    #             Pros: Moderately fast and resolves most non-unique cases.
    #             Cons: Overlooks some useful entries that are not listed first. May also pick up some redundant entries.
    #         2:
    ...
    #     TODO:   Test each mode while comparing the number of pruned matches.
    #             I should focus particularly on which matches are gained between mode 0 and mode 1, 2, 3
    #     """
    #     def process_first_match_only(handler: SearchWordMatchesHandler, result: DictionaryEntryMatchList, pbar: tqdm=None):
    #         for match in handler.first_matches:
    #             if pbar is not None:
    #                 pbar.set_description(match.unique_search_word)
    #             found = False
    ...
    #     result = DictionaryEntryMatchList()
    #     if show_pbar:
    #         if mode == 0:
    #             total_iter = len(self.unique)
    #         elif mode in [1, 2, 3]:
    #             total_iter = len(self.unique) + len(self.non_unique)
    #         else:
    #             total_iter = len(self)
    #         pbar = tqdm(total=total_iter, unit='item(s)', leave=leave_pbar)
    #     else:
    #         pbar = None

    #     # Process unique matches first
    #     process_first_match_only(handler=self.unique, result=result, pbar=pbar)
    ...
    #         # Process only first entry of non-unique.
    #         # Ignore if entry already exists in matches.
    #         process_first_match_only(handler=self.non_unique, result=result, pbar=pbar)
    #     elif mode == 2:
    #         # Process every entry of non-unique.
    #         # Ignore if any entry already exists in matches.
    #         # Otherwise add first entry.
    #         for sw_matches in self.non_unique:
    #             if pbar is not None:
    #                 pbar.set_description(sw_matches.search_word)
    #             found = False
    #             for item in result:
